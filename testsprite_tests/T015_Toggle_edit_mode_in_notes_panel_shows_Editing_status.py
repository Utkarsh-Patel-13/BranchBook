...
async def run_test():
    ...
    try:
        ...
        # -> Click on the 'HasNodes' workspace card to open that workspace (element index 3891).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/main/div[3]/div[3]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Show notes only' tab in the workspace header (index 9914), then select 'RootNode' in the sidebar (index 9851) to load its notes panel.
        frame = context.pages[-1]
        # Click element
        ...
        # -> Click the 'Show notes only' tab (index=10197) to switch to Notes view, then click 'RootNode' in the sidebar (index=10126) to load its notes panel.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/main/header/div[2]/div[2]/div/div/button[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        # -> Click the edit-mode toggle in the notes header to enable editing (click element index 14546).
        frame = context.pages[-1]
        # Click element
        ...
        editing_el = frame.locator('xpath=/html/body/div[1]/div[1]/div/main/div/div/div[1]/div/header/div[1]/span').nth(0)
        if await editing_el.is_visible():
            raise AssertionError("Expected initial header text 'Read Only' but found 'Editing'; cannot verify toggle because initial 'Read Only' state is missing. Task marked done.")
