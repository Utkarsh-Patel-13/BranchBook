...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email and password fields and click the Sign In button to authenticate and return to /workspaces.
        ...
        # -> Navigate to /login and wait for the login form to render so credentials can be submitted.
        await page.goto("http://localhost:3001/login", wait_until="commit", timeout=10000)
        
        # -> Click the 'HasNodes' workspace card to open the workspace (use interactive element [6414]).
        frame = context.pages[-1]
        # Click element
        ...
        assert '/workspaces/' in frame.url
        await expect(frame.locator('text=RootNode').first).to_be_visible(timeout=3000)
